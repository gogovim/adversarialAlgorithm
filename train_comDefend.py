...
def valid_epoch(model,valid_dataloader, lam,print_freq=40,classify_model=None):
    ...
    for i,(images,labels,noise) in enumerate(valid_dataloader):
        ...
        l2,perception_loss=cal_loss2(images,outputs).mean(),cost(y2,y1).mean()
        if i%print_freq==0:
...
def main(args):
    ...
    if args.mode == "train":
        ...
        train_data = Dataset_noise(train_dir,transform=train_transforms,shape=[18,299,299])
        train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True,num_workers=4)

        #交叉验证
        valid_data = Dataset_noise(valid_dir,transform=valid_transforms,shape=[18,299,299])
