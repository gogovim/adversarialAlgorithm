...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, norm_layer=None,activate=True):
...
class ResNet_noise(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=110, zero_init_residual=False,
        ...
        self.layer4 = self._make_layer(block, planes[3], layers[3], stride=2, groups=groups, norm_layer=norm_layer,activate=False)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, groups=1, norm_layer=None,activate=True):
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        downsample = None
        ...
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=groups, norm_layer=norm_layer,activate=(True if activate or _!=blocks-1 else False)))
    ...
    def forward(self, x,noise=None):
        ...
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        #print(x.mean().item(),noise.mean().item())
        #print(noise)
        #print(torch.abs(x).mean().item(),torch.abs(noise).mean().item())
        
        if noise is not None:
            #print(noise.shape,x.shape)
            #print("begin",torch.abs((x>0).float()).mean().item())
            #tmp=x
            #tmp=(tmp>0).float()
            #x=x-noise
            #x=torch.sigmoid(x)
...
def resnet101_noise(pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_noise(Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
