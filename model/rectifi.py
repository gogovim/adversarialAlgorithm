...
class Rectifi(nn.Module):
    ...
    def get_loss(self, images, ad_images, cost, flag, classify_model=None):
        ...
        print("rec:",rec_x.device,rec_x.shape)
        print("image:",images.shape,images.device,"ad_image",ad_images.shape,ad_images.device)
        loss1 = cost(images, rec_x,classify_model).mean()
        if classify_model is None:
            loss2 = cost(images, rec_xbar).mean()
        else:
            loss2 = cost(images, rec_xbar, classify_model).mean()
        print(loss1.device,loss2.device,loss1.shape,loss2.shape)
