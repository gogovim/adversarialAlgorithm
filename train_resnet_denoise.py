...
def valid_epoch(model,valid_dataloader, lam,print_freq=40,classify_model=None):
    ...
    for i,(images,labels,noise) in enumerate(valid_dataloader):
        ...
        #print(images.shape,labels.shape)
        #optimizer.zero_grad()
        images=model(images,noise=noise)
        y1=classify_model(images)
        loss=cost(y1,labels).mean()
        ...
        if i % print_freq == 0:
            ...
            print(res)
    return batch_time.avg, losses.avg,error1.avg
...
def train(model,train_dataloader,lr,save_dir,num_epoches,model_name,valid_dataloader=None,classify_model=None):
    optimizer = torch.optim.SGD(classify_model.parameters(), lr=lr, momentum=0.9, nesterov=True, weight_decay=0.0005)
    ...
    for epoch in range(num_epoches):
        ...
        if valid_dataloader and valid_error< best_error:
            ...
            torch.save(classify_model.state_dict(), os.path.join(save_dir, 'resnet101_denoise'+'_model.dat'))
        else:
            nobetter_num+=1

        with open(os.path.join(save_dir, 'resnet101_denoise'+'_results.csv'), 'a') as f:
...
def main(args):
    model=ComDefend(pho_size=args.pho_size)
    ...
    if args.mode == "train":
        ...
        train_data = Dataset_noise(train_dir,transform=train_transforms,shape=[12,299,299])
        train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True,num_workers=4)

        #交叉验证
        valid_data = Dataset_noise(valid_dir,transform=valid_transforms,shape=[12,299,299])
        ...
        train(model,train_loader,args.lr,args.save_dir,args.epoch,model_name,valid_loader,classify_model=classify_model)
...
if __name__=="__main__":
    ...
    batch_size=2
    #./weights/comdefend_299_model.dat
    parser = argparse.ArgumentParser(description='ComDefend')
    ...
    #load model
    parser.add_argument('--model_name', type=str, default='comdefend', help='model name')
    parser.add_argument('--pre_train', type=str, default='./weights/comdefend_299_model.dat', help='weights')
